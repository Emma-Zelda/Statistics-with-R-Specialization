...
sfo_feb_flights %>%
  ...
  geom_vline(xintercept = median(sfo_feb_flights$arr_delay), color = 'red')
```
...
sfo_feb_flights %>%
  group_by(carrier) %>%
  summarise(median_arr_delay = median(arr_delay), IQR_arr_delay = IQR(arr_delay))
...
nycflights %>%
  ...
  summarise(ot_arr_delay_dep_rate = sum(arr_type == "on time") / n())
```
